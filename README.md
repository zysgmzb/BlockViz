...
### 1. 环境要求
- Python 3.10+
- 系统级 Qt 依赖由 PySide6 wheel 自动提供（Windows / macOS / 大部分 Linux 发行版）
- 建议联网以安装依赖，或在企业网络内配置 PyPI 镜像
...
### 3. 运行
```bash
python -m blockviz                 # 使用 setuptools entry point
# 或
blockviz                           # 安装为可执行脚本后直接调用
# 或（仅调试）
./start.sh                         # 编译 src 后在子进程中启动
```

第一次启动会提示输入 RPC URL。可直接在侧栏输入任意兼容 Ethereum JSON-RPC Endpoint 并点击 **Apply RPC**，状态栏会提示连接结果。
